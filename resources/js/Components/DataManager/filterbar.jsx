        ...
        <div id="filter-bar">
            <div className="flex flex-wrap items-center gap-2">
                <div className="relative min-w-0 flex-1" style={{ maxWidth: 280 }}>
                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        <svg className="h-3.5 w-3.5 text-brand-mid" fill="currentColor" viewBox="0 0 16 16">
                ...
                {hasBuilder && (
                    ...
                    >
                        ...
                        {activeRuleCount > 0 && (
                            <span className="absolute -right-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-brand-accent text-[9px] font-bold text-brand-white ring-2 ring-brand-white">
                                {activeRuleCount}
                            </span>
                        )}
                    </button>
            ...
            {hasBuilder && builderOpen && (
                <div className="mt-3 rounded-lg border border-brand-mid/20 bg-brand-light p-4">
                            ...
                            {['and', 'or'].map((l) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 transition-colors ${
                                        logic === l ? 'bg-brand-accent text-brand-white' : 'bg-transparent text-brand-mid hover:bg-brand-light'
                                    }`}
                                >
                    ...
                    <div className="flex flex-col gap-2">
                        {rules.map((rule, idx) => (
                            <div key={idx} className="flex items-center gap-2">
                                <span className="w-8 shrink-0 text-center text-xs font-bold text-brand-mid">
                                    {idx === 0
                    ...
                    <div className="mt-3 flex items-center gap-2 border-t border-brand-mid/20 pt-3">
                        <button
                            type="button"
                            onClick={addRule}
